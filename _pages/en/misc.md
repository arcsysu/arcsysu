---
title: "arcSYSu Lab - Misc"
lang: en
layout: textlay
excerpt: "arcSYSu Lab -- Misc"
sitemap: false
zh_url: /zh/misc
permalink: /misc
---

# Miscellaneous

## Artifacts and Services
Yat Creative Cloud (<a href="https://yatcc-ai.com/" target="_blank">YatCC</a>)<br>
 - ⚙️ YatCC <a href="https://ol.yatcc.arcsysu.cn/" target="_blank">OL Lab</a><br>
 - 🤖 YatCC <a href="https://portal.aihub.arcsysu.cn/" target="_blank">AI Hub</a>

<a href="https://yatcc-ai.com/" target="_blank"> ![]({{ site.url }}{{ site.baseurl }}/images/misc/yatcc.webp){: style="width: 50%; margin: 30px"} </a>

## Follow Us
Learn more about our research, projects and related activities.
<br><br>

<a href="https://github.com/arcsysu" target="_blank"> ![]({{ site.url }}{{ site.baseurl }}/images/misc/GitHub.svg){: style="height: 40px; float: center; margin: 15px; padding-right:20px"} </a>
<a href="https://yatcc-ai.com/" target="_blank"> ![]({{ site.url }}{{ site.baseurl }}/images/misc/Bilibili.svg){: style="height: 40px; float: center; margin: 15px; padding-right:20px"} </a>
<a href="https://www.xiaohongshu.com/user/profile/68b412c2000000001900f4fe" target="_blank"> ![]({{ site.url }}{{ site.baseurl }}/images/misc/RedNote.webp){: style="height: 30px; float: center; margin: 15px"} </a>
<br>    
<p class="misc"></p>


## Join Us
Shape the future of computing together with us. See <a href="{{ site.url }}{{ site.baseurl }}/files/faq.html" target="_blank">FAQs</a>.
<br>
<a href="https://xianweiz.github.io/notes/faq.html" target="_blank"> ![]({{ site.url }}{{ site.baseurl }}/images/misc/join.webp){: style="height: 70px; float: center; margin: 15px"} </a>
